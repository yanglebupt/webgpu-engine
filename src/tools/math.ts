...
// modified from https://github.com/mrdoob/three.js/blob/dev/src/math/MathUtils.js
...
// compute euclidean modulo of x % a
...
export function pingpong(x: number, length = 1) {
  return length - Math.abs(mod(x, length * 2) - length);
}
...
export const arrayProd = (arr: number[]) =>
  arr.reduce((p: number, c: number) => p * c, 1);
